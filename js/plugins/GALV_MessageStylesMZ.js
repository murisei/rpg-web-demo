...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc (v.2.1) “显示文本”窗口浮动在角色上方，并在游戏期间更改视觉效果
 ...
 * @type boolean
 * @default false
 * @desc 当显示浮动消息时，显示窗口位置“底部”将改为顶部，反之亦然。
 *
 * @param aGraphic
 * @text 箭头图形
 ...
 * ----------------------------------------------------------------------------
 *此插件的主要目的是允许您的“显示文本”消息框
 *与游戏中的其他窗口具有不同的风格。插件设置
 *具有多种可视设置，您可以调整，此外还可以使用
 *显示文本消息中的文本代码将允许您将该消息转换为浮动消息。
 *
 * ----------------------------------------------------------------------------
 *   在SHOW TEXT消息事件命令中使用的代码
 * ----------------------------------------------------------------------------
 * 以下代码必须放在Show Message事件的第一行
 *
 *       \pop[t]
 *
 * t = 事件Id-在地图上以事件为目标
 *     0 to将“显示文本”命令用于的事件作为目标
 ...
 * 示例
 * \pop[23]  // 目标事件23
 * \pop[-3]  // 瞄准你的随从阵容中的第三个随从
 * \pop[a8]  // 仅当演员是地图上的跟随者时，才以演员8为目标
 * \pop[500|200] // 将消息中心对准屏幕位置
 *               // 500px X and 200px Y
 *
 * OPTIONALLY:
 * 可以通过消息中的\pop[]文本添加高级数据。
 *
 *       \pop[t,y,windowskin,windowarrow]
 *
 * t      = same as above
 * y      = 另一个以像素为单位的y偏移修改器-用于更高的
 *          设置为0以不修改默认的y偏移
 ...
 *   插件命令
 * ----------------------------------------------------------------------------
 * 有一些插件命令可用于更改windowskins。当他们是
 * 使用，所有邮件将使用新设置，直到再次使用
 * 将它们改回默认值。这些是正在进行的工作，更多功能
 * 可以添加到它们中。
 *
 */
//-----------------------------------------------------------------------------
